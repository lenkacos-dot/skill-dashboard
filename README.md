...
| 模块 | 说明 |
|------|------|
| **💰 性价比分析** | `cost_efficiency = 健康分/磁盘占用`, 量化投入产出比 |
| **⏰ 冷启动标识** | 5 级冷启动标签（🔥近期→❄️从未），一眼识别吃灰 skill |
| **🔧 自动优化建议** | 6 类问题诊断：闲置/臃肿/报错/重叠/低效 |
| **📦 磁盘占用统计** | 扫描每个 skill 实际磁盘占用 + KB/MB 格式化 |
...
| `report.py order` | 最优加载顺序（含性价比/磁盘/冷启动） | "哪个先加载" |
| `report.py suggest` | 自动优化建议（6 类问题诊断） | "优化建议/精简" |
| `report.py cost` | 磁盘占用 & 性价比 & 冷启动 | "磁盘占用/性价比" |
